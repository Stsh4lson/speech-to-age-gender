...
import settings
settings.init()
from EncoderGenerators import TrainEncoderGenerator, ValidationEncoderGenerator
...
input = tf.keras.layers.Input(shape=(settings.AE_TIMESTEPS, settings.AE_N_FEATURES))

encoded = tf.keras.layers.LSTM(settings.AE_N_FEATURES//4, activation='tanh', return_sequences=True)(input)
encoded = tf.keras.layers.LSTM(settings.AE_LATENT_DIM, activation='tanh')(encoded)
decoded = tf.keras.layers.RepeatVector(settings.AE_TIMESTEPS)(encoded)
decoded = tf.keras.layers.LSTM(settings.AE_N_FEATURES//4, activation='tanh', return_sequences=True)(decoded)
decoded = tf.keras.layers.LSTM(settings.AE_N_FEATURES, return_sequences=True)(decoded)
...
with tf.device("GPU:0"):
    autoencoder.fit(
        ...
        validation_data = ValidationEncoderGenerator()
            ...
            .map(scaled, num_parallel_calls=tf.data.experimental.AUTOTUNE)
            .map(doubleOutput, num_parallel_calls=tf.data.experimental.AUTOTUNE),
        validation_steps = len(settings.AE_VALIDATION_IDX) // settings.AE_BATCH_SIZE,
        callbacks = callbacks
        )
